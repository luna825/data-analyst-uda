...
#### 固定酸度图
```{r}
ggplot(aes(x = fixed.acidity), data=pf) + geom_histogram(binwidth = 0.5) + scale_x_continuous(breaks = seq(4,16, 0.5))
```
...
#### 挥发物酸度
```{r}
ggplot(aes(x = volatile.acidity), data = pf ) + geom_histogram(binwidth = 0.02) + scale_x_continuous(breaks = seq(0,1.6, 0.2))
```
...
```{r}
grid.arrange(ggplot(aes(x = fixed.acidity), data =pf ) + geom_histogram() + scale_x_log10(),
             ggplot(aes(x = volatile.acidity), data =pf ) + geom_histogram() + scale_x_log10(),
             ggplot(aes(x = citric.acid), data =pf ) + geom_histogram() + scale_x_log10(), ncol = 1
             )
```
...
#### 与质量的关系
从箱线图中可以看出，fixed acidity, citric acid, sulphates ， alcohol 与葡萄酒质量直接相关，volatile acidity ， pH 跟质量是负相关的。 从相关性测试中，观察到类似的趋势；
